...
// 🎯 উন্নত Hash ফাংশন (djb2) - দ্রুত এবং রিলায়েবল আইডি তৈরির জন্য
...
async function handleRequest(req: Request) {
  try {
    ...
    // 🎯 ১০০% পারফেক্ট POST ডাটা রিড (Form Data এবং URL Encoded উভয়ের জন্য)
    if (req.method === 'POST') {
    ...
    if (!action) {
      return NextResponse.json({
        user_info: {
          ...
          auth: 1,
          status: "Active",
          exp_date: user.premiumExpiry ? Math.floor(new Date(user.premiumExpiry).getTime() / 1000).toString() : "1999999999",
      ...
      if (mergedM3uDoc && mergedM3uDoc.content) {
        ...
        for (let line of lines) {
          if (line.startsWith('#EXTINF')) {
            ...
            if (match) categoriesMap.set(match[1], generateId(match[1]));
          }
        }
      }
    ...
    if (action === 'get_live_streams') {
      ...
      const processedUrls = new Set(); // 🚀 ম্যাজিক ফিক্স: ডুপ্লিকেট চ্যানেল রিমুভার
    ...
    // 🎯 VOD / Series Fallback (খুবই জরুরি, নইলে অ্যাপ ক্র্যাশ করে)
