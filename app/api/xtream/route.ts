...
async function handleXtreamRequest(req: Request) {
  try {
    ...
    if (AUTH_CACHE[cacheKey] && AUTH_CACHE[cacheKey].expiry > now) {
      ...
      AUTH_CACHE[cacheKey] = { valid: isUserValid, expiry: now + 5 * 60 * 1000, userId: user ? user._id.toString() : '' };
    }
    ...
    if (!STREAM_CACHE || !CATEGORY_CACHE || (now - CACHE_TIME) > CACHE_TTL) {
      ...
      streams.forEach((stream) => {
        ...
        processedStreams.push({
          ...
          container_extension: "m3u8", // 🎯 CRITICAL FIX: প্লেয়ারকে জানানো হলো ভিডিওটি m3u8
          custom_sid: "",
          tv_archive: 0,
          direct_source: "",
        ...
        for (let line of lines) {
          ...
          if (line.startsWith('#EXTINF')) {
            ...
            processedStreams.push({
              ...
              added: Math.floor(now / 1000).toString(),
              category_id: numericCatId,
              container_extension: "m3u8", // 🎯 CRITICAL FIX
              custom_sid: "",
              tv_archive: 0,
              direct_source: "",
              tv_archive_duration: 0
      ...
      if (processedStreams.length === 0) {
        ...
        processedStreams.push({
          num: 1, name: "No Channels Available", stream_type: "live", stream_id: 100, stream_icon: "", epg_channel_id: null, added: "0", category_id: "1", container_extension: "m3u8", custom_sid: "", tv_archive: 0, direct_source: "", tv_archive_duration: 0
        });
      }

      const processedCategories = Array.from(categoriesMap.entries()).map(([catId, catName]) => ({
        category_id: catId, category_name: catName, parent_id: 0
      }));
    ...
    if (!action) {
      return NextResponse.json({
        user_info: {
          username: username, password: password, message: "Logged In", auth: 1, status: "Active", exp_date: "1799345858", is_trial: "0", active_cons: "0", created_at: Math.floor(now / 1000).toString(), max_connections: "2", allowed_output_formats: ["m3u8", "ts", "rtmp"]
        },
        server_info: {
          url: host, port: "80", https_port: "443", server_protocol: protocol, rtmp_port: "8080", timezone: "Asia/Dhaka", timestamp_now: Math.floor(now / 1000), time_now: new Date().toISOString()
        }
      }, { headers: corsHeaders });
    }
